...
public class Common {
	...
	private Map<String, User> loadUserDb(File dbFile) throws IOException, DatabaseException {
	...
	public User requireLoggedInUser(HttpServletRequest request, HttpServletResponse response) throws IOException {
	...
	public DbxRequestConfig getRequestConfig(HttpServletRequest request) {
		return DbxRequestConfig.newBuilder("example-web-file-browser").withUserLocaleFrom(request.getLocale()).build();
	}

	public void handleDbxException(HttpServletResponse response, User user, DbxException ex, String action)
			throws IOException {
		// The one special error is InvalidAccessToken. This is probably
		// happening because
		// the user "unlinked" us through Dropbox's website. Clear their access
		// token in the
		// database (since it's invalid anyway).
		if (ex instanceof InvalidAccessTokenException) {
			page(response, 400, "Cannot access Dropbox account",
					"It looks like your web-file-browser account is no longer linked to your Dropbox account.");
	...
	public void handleException(HttpServletResponse response, Exception ex, String action) throws IOException {
		...
		// - If it's Dropbox's fault, their service will eventually get fixed
		// and the retry will work (sooner or later).
		// - If it's our fault, we might fix our service and the retry will work
		// eventually.
		page(response, 503, "Error communicating with Dropbox", "Try again later?");

		// But obviously we should still log all these errors because they
		// probably indicate a real problem.
		// - If it's Dropbox's fault, we should record error information so we
		// can notify Dropbox
